...
clean :
	ocamlbuild -clean
	rm -rf testall.log *.diff fire scanner.ml parser.ml parser.mli
	rm -rf printbig
...
#
TARFILES = ast.ml codegen.ml Makefile fire.ml parser.mly README scanner.mll \
	semant.ml testall.sh $(TESTFILES:%=tests/%) printbig.c arcade-font.pbm \
	font2c

fire-llvm.tar.gz : $(TARFILES)
	cd .. && tar czf fire-llvm/fire-llvm.tar.gz \
		$(TARFILES:%=fire-llvm/%)
